...
const CardAnime = ({ anime }) => {
  ...
  const fetchMessages = async (groupId) => {
    try {
      ...
      const formattedMessages = data.map(msg => ({
        ...
        senderName: msg.User?.name || `Usuário ${msg.senderId}`, // Assumindo que a API inclui dados do usuário
        createdAt: new Date(msg.createdAt),
        isCurrentUser: msg.senderId === user.id // Função para obter ID do usuário atual
      }));
  ...
  useEffect(() => {
    if (isModalOpen && id) {
      fetchMessages(id); // Usando o ID do anime como groupId
    }
  }, [isModalOpen, id]);
  ...
  const closeModal = () => {
    ...
    setError(null); // Limpar erros ao fechar
  };
